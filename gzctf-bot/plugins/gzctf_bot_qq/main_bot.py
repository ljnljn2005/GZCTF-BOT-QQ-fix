...
async def _():
    ...
    try:
        ...
        print(f"[DEBUG] 没有可用的 bot 实例: {e}")
        return
    
    if STATUS:
        print(f"[DEBUG] 定时任务开始执行，当前时间: {datetime.now(UTC8)}")
        
        if not CONFIG.get("GAME_LIST"):
            tmpGameInfo = getGameList()
            if tmpGameInfo != GAME_LIST:
                print(f"[DEBUG] 比赛列表有更新，旧: {len(GAME_LIST)} 项，新: {len(tmpGameInfo)} 项")
        ...
        for gameInfo in GAME_LIST:
            print(f"[DEBUG] 检查比赛: {gameInfo['title']} (ID: {gameInfo['id']})")
            
            # 检查比赛通知
            tmpGameNotice = getGameNotice(gameInfo['id'])
            print(f"[DEBUG] 比赛 {gameInfo['title']} 的通知数量: {len(tmpGameNotice)}")
            ...
            if tmpGameNotice != GAMENOTICE[f"gameId_{str(gameInfo['id'])}"]:
                ...
                print(f"[DEBUG] 发现 {len(tmpNotice)} 条新通知")
                ...
                for newNotice in tmpNotice:
                    ...
                    print(f"[DEBUG] 处理通知类型: {msgType}, 内容: {msgContent}")
                    ...
                    if msgType in msgList.keys():
                        if msgType == "FirstBlood" or msgType == "SecondBlood" or msgType == "ThirdBlood":
                            ...
                            print(f"[DEBUG] 准备发送新题目消息: {msg}")
                        else:
                            msg = msgTemp_all.format(type=msgList[msgType], gameName=gameInfo['title'],
                                                     time=f"{msgTime[0]}-{msgTime[1]}-{msgTime[2]} {msgTime[3]}:{msgTime[4]}:{msgTime[5]}",
                                                     content=msgContent[0])
                            print(f"[DEBUG] 准备发送普通消息: {msg}")
                        
                        for id in SEND_GAME_LIST[gameInfo['title']]:
                            try:
                                print(f"[DEBUG] 尝试向群 {id} 发送消息")
                                await bot.send_group_msg(group_id=id, message=msg)
                                print(f"[DEBUG] 消息发送成功到群 {id}")
                            except Exception as e:
                                print(f"[ERROR] 发送消息到群 {id} 失败: {e}")
                                await bot.send_group_msg(group_id=id, message="Error")
            
            # 检查作弊信息（如果启用了自动封禁）
            if BAN_STATUS:
                print(f"[DEBUG] 检查比赛 {gameInfo['title']} 的作弊信息")
                tmpGameCheats, code = getCheatInfo(gameInfo['id'])
                
                if code == 400:
                    print(f"[DEBUG] 比赛 {gameInfo['title']} 返回 400，跳过作弊检查")
                    continue
                
                print(f"[DEBUG] 比赛 {gameInfo['title']} 的作弊信息数量: {len(tmpGameCheats)}")
                ...
                if tmpGameCheats != GAMECHEATS[f"gameId_{str(gameInfo['id'])}"]:
                    ...
                    print(f"[DEBUG] 发现 {len(tmpCheats)} 条新作弊信息")
                    ...
                    for newCheat in tmpCheats:
                        ...
                        print(f"[DEBUG] 准备发送封禁消息: {msg}")
                        print(f"[DEBUG] 封禁队伍ID: {teamIds}")
                        
                        banTeam(teamIds)
                        for id in SEND_GAME_LIST[gameInfo['title']]:
                            try:
                                print(f"[DEBUG] 尝试向群 {id} 发送封禁消息")
                                await bot.send_group_msg(group_id=id, message=msg)
                                print(f"[DEBUG] 封禁消息发送成功到群 {id}")
                            except Exception as e:
                                print(f"[ERROR] 发送封禁消息到群 {id} 失败: {e}")
                                await bot.send_group_msg(group_id=id, message="Error")
